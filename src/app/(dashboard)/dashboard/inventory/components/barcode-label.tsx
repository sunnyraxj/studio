    ...
    return (
        ...
        >
            <div className="w-full flex justify-center items-center relative top-[2%]">
                <p className="text-[8px] uppercase">{shopName}</p>
                {isQuickBarcode && (
                    <span className="absolute right-0 text-[8px] uppercase font-bold">Quick</span>
                )}
            </div>
            
            <div className="w-full flex-grow flex flex-col justify-center items-start gap-0 px-1">
                 <p className="text-[10px] truncate max-w-full relative">{item.name}</p>
